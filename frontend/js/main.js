...
function showUsageTree(itemId) {
    ...
    modalContent.innerHTML = `
        ...
        <h3 class="text-xl font-bold mb-4 text-center">${item.name} - 用途树</h3>
        <div class="synthesis-tree">
...
function renderUsageTreeNode(parentContainer, itemId, quantity, isRoot = false, sourceRecipe = null) {
    ...
    let nodeHTML = `
        <img src="${imageUrl}" alt="${item.name}">
        <div class="node-info">
            <div class="node-name">${item.name} × ${quantity}</div>
    `;
    
    if (!hasUsage) {
        nodeHTML += `<div class="node-acquisition">(最终装备)</div>`;
    } else {
        nodeHTML += `<div class="node-acquisition">可用于合成 ${usageRecipes.length} 种装备</div>`;
    }
    
    // 如果有来源配方，显示这个物品是通过什么配方合成的
    if (sourceRecipe) {
        let materialsHTML = '';
        sourceRecipe.materials.forEach((material, index) => {
            ...
            if (materialItem) {
                ...
                const acquisitionMethod = materialItem.acquisitionMethod || '未知';
                
                // 检查这个材料是否在升级链条中（包括原始物品和所有上位装备）
        ...
        nodeHTML += `
            <div class="recipe-formula-inline">
                <div class="formula-title">配方:</div>
    ...
    materialElement.addEventListener('mouseenter', (e) => {
        ...
        showTimeout = setTimeout(() => {
            ...
            tooltip.innerHTML = `
                ...
                <div class="tooltip-acquisition">获取方式: ${materialAcquisition}</div>
            `;
