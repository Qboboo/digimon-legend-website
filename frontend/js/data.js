...
let digimonData = [];
let evolutionData = {};
let guidesData = {}; // For categorized guides: { gameplay: [], area: [], boss: [] }
...
async function initData() {
    console.log('Initializing all data from APIs...');
    try {
        ...
        const fetchPromises = [
            ...
            fetch(`${API_BASE_URL}/guides?${cacheBuster}`),
        ...
        // Categorize guides for easier access
        if (guides) {
            guidesData = {
...
function getAllGuides() {
    const all = [
        ...(window.guidesData?.gameplay || []),
        ...(window.guidesData?.area || []),
        ...(window.guidesData?.boss || [])
    ];
    return all.sort((a, b) => new Date(b.updateDate || b.updatedAt) - new Date(a.updateDate || a.updatedAt));
}
